...
lexers = {
    "sql": PygmentsLexer(GoogleSqlLexer),
...
class ZMQTerminalInteractiveShell(SingletonConfigurable):
    ...
    def __init__(self, **kwargs):
        ...
        # self.pt_cli.app.vi_state.input_mode = InputMode.NAVIGATION
    ...
    def vi_mode(self):
        if getattr(self, "editing_mode", None) == "vi" and self.prompt_includes_vi_mode:
            return "[" + str(self.pt_cli.app.vi_state.input_mode.value)[3:6] + "] "
        return ""
    ...
    def continuations(self, width, lineno, is_soft_wrap):
        return PygmentsTokens(self.get_continuation_tokens(width))
    ...
    def init_prompt_toolkit_cli(self):
        ...
        @kb.add("f8")
        def _(event):
            self.pt_cli.lexer = lexers["python"] if self.pt_cli.lexer == lexers["sql"] else lexers["sql"]
        ...
        self.pt_cli = PromptSession(
            ...
            lexer=lexers["python"],
            prompt_continuation=self.continuations,
            key_bindings=kb,
            history=history,
    ...
    def run_cell(self, cell, store_history=True):
        ...
        if cell.strip().split()[0] in sqlterms:
            cell = f'df = pd.DataFrame(dict(row) for row in bq_client.query("""\n\n{cell}\n\n""").result())\ndf'
            # store_history = False
            # from sqlrepl import embed
